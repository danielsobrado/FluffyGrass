...
function periodicWorleyRidge(
  ...
  for (let offsetY = -1; offsetY <= 1; offsetY += 1) {
    for (let offsetX = -1; offsetX <= 1; offsetX += 1) {
      ...
      const pointX = cellX + 0.12 + hash(wrappedX, wrappedY, seed) * 0.76;
      const pointY =
        cellY + 0.12 + hash(wrappedX, wrappedY, seed + 19) * 0.76;
...
export function sampleWaterFlowNoisePixel(
  ...
  const wrappedX = wrap(x, WATER_FLOW_NOISE_SIZE);
  const wrappedY = wrap(y, WATER_FLOW_NOISE_SIZE);
  const broad =
    periodicValueNoise(wrappedX, wrappedY, 4, seed + 11) * 0.7 +
    periodicValueNoise(wrappedX, wrappedY, 9, seed + 17) * 0.3;
  const turbulence =
    periodicValueNoise(wrappedX, wrappedY, 17, seed + 23) * 0.64 +
    periodicValueNoise(wrappedX, wrappedY, 31, seed + 29) * 0.36;
  const caustic =
    periodicWorleyRidge(wrappedX, wrappedY, 11, seed + 37) * 0.72 +
    periodicWorleyRidge(wrappedX, wrappedY, 23, seed + 41) * 0.28;
  const glint =
    periodicValueNoise(wrappedX, wrappedY, 43, seed + 47) * 0.52 +
    hash(wrappedX, wrappedY, seed + 53) * 0.48;
